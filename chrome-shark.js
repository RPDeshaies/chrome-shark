...
var ChromeShark = (function() {
    ...
    function ChromeShark($, chromeSharkInterface) {
        this.songTitle = "";
        this.artist = "";
        this.artistBio = "";
        this.timeElapsed = "";
        ...
        function InitializePlayerInfo(){
          if(!this.playerInfo || !this.playerInfo.$song || this.playerInfo.$song.length == 0){
            ...
            if (chromeSharkInterface == ChromeShark.prototype.Interface.Grooveshark) {
                this.playerInfo = {
        ...
        function UpdateMetaData() {
          ...
          if(playerExists){
            ...
            var newTimeElapsed = thisRef.playerInfo.$timeElapsed.text();
            var newTimeTotal = thisRef.playerInfo.$timeTotal.text();
        ...
        function GetLastFmData() {
            ...
            $.ajax("https://ws.audioscrobbler.com/2.0/?" + "method=track.getInfo&" + "api_key=" + properties.lastFmApiKey + "&" + "artist=" + encodeURI(this.artist) + "&" + "track=" + encodeURI(this.songTitle) + "&" + "format=json", {
            ...
            $.ajax("https://ws.audioscrobbler.com/2.0/?" + "method=artist.getInfo&" + "api_key=" + properties.lastFmApiKey + "&" + "artist=" + encodeURI(this.artist) + "&" + "format=json", {
                success: function(data) {
        ...
        this.IsUIVisible = function() {
            var $chromeSharkInterface = $(".chrome-shark");
            return $chromeSharkInterface && $chromeSharkInterface.is(":visible");
        };
        /**
        ...
        function __constructor() {
            InitializePlayerInfo.call(this);
            UpdateMetaData.call(this);
        }
...
function Main() {
    if (window.chromeShark && window.chromeShark.IsUIVisible()) {
        window.chromeShark.Close();
    } else {
        var chromeSharkInterface = ChromeShark.prototype.Interface.Grooveshark;
        if (location.host.indexOf("grooveshark") == -1) {
            chromeSharkInterface = ChromeShark.prototype.Interface.Spotify;
        }
        window.chromeShark = window.chromeShark || new ChromeShark(jQuery, chromeSharkInterface);
        window.chromeShark.Show();
    }
}
